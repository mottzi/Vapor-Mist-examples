...
extension Application {
    func useMistDemo() {
        ...
        self.get("DemoModel1", "update", ":id", ":text") { req async throws -> HTTPStatus in
        ...
        self.get("DemoModel2", "update", ":id", ":text") { req async throws -> HTTPStatus in
            guard let idString = req.parameters.get("id"),
                let id = UUID(uuidString: idString)
            else { throw Abort(.badRequest, reason: "Valid UUID parameter is required") }
        ...
        self.get("DemoModels", "delete", ":id") { req async throws -> HTTPStatus in
            guard let idString = req.parameters.get("id"),
                let id = UUID(uuidString: idString)
            else { throw Abort(.badRequest, reason: "Valid UUID parameter is required") }
        ...
        self.get("DemoModels", "deleteAll") { req async throws -> HTTPStatus in
            try await MistDemoModel1.query(on: req.db).delete()
            try await MistDemoModel2.query(on: req.db).delete()
            return .ok
        }

        self.get("DemoModels", "create") { req async throws -> MistDemoModel1 in
            let words =
                [

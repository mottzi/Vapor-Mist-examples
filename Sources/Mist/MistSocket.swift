...
public struct Socket {
    func register(on app: Application) {
        app.webSocket(app.mist.socketPath) { request, ws async in
...
extension Socket.Connection {
    ...
    func handleAction(_ component: String, _ id: UUID?, _ action: String) async {
        ...
        if case .success = result, let id {
            if let componentInstance = await app.mist.components.getComponent(usingName: component)
                as? any InstanceComponent
            {
                let state = await app.mist.clients.state(
                    for: clientID, componentID: id.uuidString,
                    default: componentInstance.defaultState)
                if let html = await componentInstance.render(
                    id: id, state: state, on: app.db, using: app.leaf.renderer)
                {
                    await app.mist.clients.send(
                        Message.InstanceUpdate(component: component, id: id, html: html),
                        to: clientID)
                }
            }
        }
        ...
        let message = Message.ActionResult(
            component: component, id: id, action: action, result: result, message: resultMessage)

...
public actor Components {
    ...
    // NEW: Track active actions to prevent race conditions
...
extension Components {
    func registerComponents(_ components: [any Component], with app: Application) {
        for component in components {
            ...
            default:
                app.logger.warning(
                    "Invalid Component '\(component.name)' attempted registration: ignored.")
                continue
            }
    ...
    func performAction(
        component: String, action: String, id: UUID?, clientID: UUID, clients: MistClients,
        on db: Database
    ) async -> ActionResult {
        ...
        guard !activeRequests.contains(lockKey) else {
            return .failure(message: "Action already in progress")
        }
        ...
        guard let componentActions = componentActions[component] else {
            return .failure(message: "Component '\(component)' not found")
        }
        guard let action = componentActions[action] else {
            return .failure(message: "Action '\(action)' not found")
        }
        ...
        var state = await clients.state(
            for: clientID, componentID: componentKey, default: componentInstance.defaultState)

...
enum Message: Codable {
    ...
    case actionResult(
        component: String, id: UUID?, action: String, result: Mist.ActionResult, message: String)
...
extension Clients {
    ...
    func send(_ message: String, to clientID: UUID) {
        send(Message.Text(message: message), to: clientID)
    }
    func send(_ message: Message.Text, to clientID: UUID) { send(message: message, to: clientID) }
    func send(_ actionResult: Message.ActionResult, to clientID: UUID) {
        send(message: actionResult, to: clientID)
    }
    func send(_ create: Message.InstanceCreate, to clientID: UUID) {
        send(message: create, to: clientID)
    }
    func send(_ update: Message.InstanceUpdate, to clientID: UUID) {
        send(message: update, to: clientID)
    }
}
...
extension Message {
    ...
    struct ActionResult: SendableMessage {
        ...
        var wireFormat: Message {
            .actionResult(
                component: component, id: id, action: action, result: result, message: message)
        }
    }
}
...
extension Message {
    struct InstanceCreate: BroadcastableMessage, SendableMessage {
        ...
        let html: String
        var wireFormat: Message {
            .createInstanceComponent(component: component, id: id, html: html)
        }
    }
    ...
    struct InstanceUpdate: BroadcastableMessage, SendableMessage {
        ...
        let html: String
        var wireFormat: Message {
            .updateInstanceComponent(component: component, id: id, html: html)
        }
    }

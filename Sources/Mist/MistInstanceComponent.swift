        ...
        for subscriber in await app.mist.clients.subscribers(of: name) {
            Task.detached {
                let state = await app.mist.clients.state(
                    for: subscriber.id, componentID: id.uuidString, default: defaultState)
                guard
                    let html = await render(
                        id: id, state: state, on: app.db, using: app.leaf.renderer)
                else { return }
                await app.mist.clients.send(
                    Message.InstanceCreate(component: name, id: id, html: html), to: subscriber.id)
            }
        }
    }
    ...
    public func handleUpdate(id: UUID, app: Application) async {
        for subscriber in await app.mist.clients.subscribers(of: name) {
            Task.detached {
                ...
                await app.mist.clients.send(
                    Message.InstanceUpdate(component: name, id: id, html: html), to: subscriber.id)
            }
        }
    }
